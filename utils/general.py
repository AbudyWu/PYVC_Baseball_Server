...
class ResumeArgumentParser():
    ...
    #by 阿布弟 0708改
    def __init__(self, param_dict):
        self.model_name = param_dict.get('model_name', 'TrackNet')
        self.seq_len = param_dict.get('seq_len', 8)
        self.epochs = param_dict.get('epochs', 30)
        self.batch_size = param_dict.get('batch_size', 10)
        self.optim = param_dict.get('optim', 'Adam')
        self.learning_rate = param_dict.get('learning_rate', 0.001)
        self.lr_scheduler = param_dict.get('lr_scheduler', '')
        self.bg_mode = param_dict.get('bg_mode', '')
        self.alpha = param_dict.get('alpha', -1)
        self.frame_alpha = param_dict.get('frame_alpha', -1)
        self.mask_ratio = param_dict.get('mask_ratio', 0.3)
        self.tolerance = param_dict.get('tolerance', 4)
        self.resume_training = param_dict.get('resume_training', False)
        self.seed = param_dict.get('seed', 13)
        self.save_dir = param_dict.get('save_dir', 'exp')
        self.debug = param_dict.get('debug', False)
        self.verbose = param_dict.get('verbose', False)
...
def generate_frames(video_file):
    ...
    assert video_file[-4:] == '.mp4' or '.MP4', 'Invalid video file format.'
